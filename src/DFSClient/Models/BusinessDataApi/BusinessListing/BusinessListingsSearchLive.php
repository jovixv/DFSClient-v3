...
class BusinessListingsSearchLive extends AbstractModel {
	...
	protected function initCustomFunctionForPaths(): array {
		return [
			'tasks->(:number)->result->(:number)->items->(:number)->attributes->available_attributes' => function($key, $value) {
				return (array)$value;
			},
			'tasks->(:number)->result->(:number)->items->(:number)->attributes->unavailable_attributes' => function($key, $value) {
				return (array)$value;
			},
			'tasks->(:number)->result->(:number)->items->(:number)->place_topics' => function($key, $value) {
				return (array)$value;
			},
		];
	}
