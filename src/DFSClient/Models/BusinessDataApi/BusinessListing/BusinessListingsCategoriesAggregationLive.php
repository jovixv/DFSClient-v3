...
class BusinessListingsCategoriesAggregationLive extends AbstractModel
{
    ...
    {
        return [
            'tasks->(:number)->result->(:number)->items->(:number)->aggregation->top_categories'   => fn ($key, $value) => (array) $value,
            'tasks->(:number)->result->(:number)->items->(:number)->aggregation->top_countries'    => fn ($key, $value) => (array) $value,
            'tasks->(:number)->result->(:number)->items->(:number)->aggregation->top_attributes'   => fn ($key, $value) => (array) $value,
            'tasks->(:number)->result->(:number)->items->(:number)->aggregation->top_place_topics' => fn ($key, $value) => (array) $value,
        ];
    }
